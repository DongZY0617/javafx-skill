...
> Issues are sorted in descending severity order (Critical -> Major -> Minor -> Info); within the same level, sorted by verification dimension (Compile -> Runtime -> Packaging). Multiple manifestations of the same root cause are merged into one issue.
...
> **Skipped Note**: A dimension is skipped when compile verification short-circuits (compile failure prevents running uncompiled code), when the toolchain is missing (packaging cannot proceed), or when incremental/targeted mode excludes it.
