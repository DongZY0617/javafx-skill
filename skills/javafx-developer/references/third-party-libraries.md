...
// 推荐：JavaFX 原生 TextInputDialog（输入对话框）
TextInputDialog input = new TextInputDialog();
input.setTitle("输入名称");
input.setHeaderText("请输入项目名称");
...
> 旧代码兼容：ControlsFX 8.x 曾提供 `Dialogs.create().title(...).showConfirm()` 流式 API，**仅用于兼容旧代码，新项目请勿使用**。参考写法：
>
> ```java
> // 不推荐：ControlsFX 8.x 旧版流式 API（新代码请用原生 Alert / TextInputDialog）
...
public class FormController {
    ...
    // 保存每个控件对应的错误 Tooltip 引用，便于卸载时传入正确实例
    ...
    /**
     * 错误装饰器：为控件添加红色边框和提示。
     * 关键点：Tooltip 实例保存到成员 Map 中，remove 时传入同一引用卸载，
     * 不能传 null（否则无法移除已安装的 Tooltip）。
     */
    private Decoration decorateError(ValidationMessage message) {
        return new Decoration() {
            ...
            public void remove(Node target) {
                target.setStyle("");
                // 卸载时必须传入之前 install 的同一 Tooltip 实例
...
> 注意：`Tooltip.uninstall(Node, Tooltip)` 的第二个参数必须传入先前 `Tooltip.install` 时使用的同一 `Tooltip` 实例；传 `null` 不会卸载任何 Tooltip，是常见错误。
...
AtlantaFX 是现代 Flat 风格主题库，内置多套高质量主题（如 Casper、Primer 等），为 JavaFX 标准控件提供统一、精致的扁平化外观，开箱即用。

### 9.1 主要特性

| 特性            | 说明                                                        |
|-----------------|-------------------------------------------------------------|
| Flat 风格主题   | 内置 Casper、Primer Light/Dark、Atlanta Dark 等多套主题     |
| 零 CSS          | 直接替换用户代理样式表，无需手写 CSS                         |
| 高保真控件      | 覆盖几乎所有标准 JavaFX 控件，含悬停/聚焦/禁用状态          |
| 亮色 / 暗色     | 通过 `Style` 枚举或不同主题类一键切换                        |
| 与 ControlsFX 兼容 | 提供对部分扩展控件的主题适配                              |
...
> 提示：AtlantaFX 通过 `setUserAgentStylesheet` 全局生效，所有标准控件会自动获得 Flat 外观，无需在 Scene 上额外加载 CSS。
...
| MaterialFX       | 11.17.0       | 兼容      | 兼容      | 已知兼容          | 需 JavaFX 17+，控件 API 差异较大 |
| RichTextFX       | 0.11.5        | 兼容      | 兼容      | 已知兼容          | 依赖 ReactFX / Flowless       |
| Ikonli           | 12.3.1        | 兼容      | 兼容      | 兼容              | 纯 Java，兼容性好             |
| ValidatorFX      | 0.4.0         | 兼容      | 兼容      | 已知兼容          | 轻量级，纯 Java               |
| TestFX           | 4.0.18        | 兼容      | 兼容      | 需 --add-opens    | 测试框架，反射访问 JavaFX 模块 |
| JMetro           | 11.6.16       | 兼容      | 兼容      | 已知兼容          | 主题库                        |
| BootstrapFX      | 0.4.0         | 兼容      | 兼容      | 兼容              | 纯 CSS，兼容性好              |
| AtlantaFX        | 2.0.1         | 兼容      | 兼容      | 已知兼容          | 需 JavaFX 17+，Flat 主题库    |
| FXGL             | 17.3          | 兼容      | 兼容      | 已知兼容          | 游戏引擎，需 JavaFX 17+       |

### 兼容性注意事项

1. **JavaFX 24+ 的 `--enable-native-access`**：部分库若依赖本地代码，可能需要额外配置 JVM 参数。
2. **`--add-opens` 参数**：像 TestFX 这类通过反射访问 JavaFX 内部成员的库，在高版本 JDK 上需添加 `--add-opens javafx.graphics/com.sun.javafx.application=ALL-UNNAMED` 等参数，否则会抛出 `InaccessibleObjectException`。
3. **模块化冲突**：非模块化库在 jlink 打包时可能报错，需通过 `--add-modules` 或将库转为自动模块处理。
4. **版本锁定**：建议在 `pom.xml` 中统一锁定 JavaFX 版本与库版本，避免传递依赖冲突。
5. **测试验证**：升级 JavaFX 版本后，务必回归测试所有第三方库功能。
...
| 需求场景                 | 推荐库                              |
|--------------------------|-------------------------------------|
| 对话框 / 通知 / 高级控件 | ControlsFX（对话框优先用原生 Alert）|
