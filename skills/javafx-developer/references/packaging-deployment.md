...
| 平台    | 必需工具                                          | 安装方式                              |
|---------|---------------------------------------------------|---------------------------------------|
| Windows | WiX Toolset 4.x（生成 msi）、Inno Setup（生成 exe）| `dotnet tool install --global wix`（v4+） |
...
| `--strip-native-commands` | 去除 java/keytool 等原生命令，进一步减小体积 |
...
```bash
# 设置 GraalVM 环境（GraalVM JDK 21+ 已内置 native-image，无需 gu install）
...
本节讨论 ARM64 架构和 macOS Universal Binary 的打包方案，覆盖 Windows ARM64 和 macOS 多架构合并两个场景。
...
```bash
# 1. 分别构建 x64 和 aarch64 的 jlink 镜像
...
# 2. 使用 jpackage 分别构建两个 app-image
...
> 提示：Gluon 的 JavaFX SDK 提供 `mac` 和 `mac-aarch64` 两种分类器，构建时需分别下载对应版本。
