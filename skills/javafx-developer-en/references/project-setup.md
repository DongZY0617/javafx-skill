...
|-------------|-----------------|----------|----------|---------------------------------------------------------------------|
| 17          | 2021-09-14      | JDK 17   | LTS      | 长期支持版本，企业级首选；稳定成熟，第三方库兼容性最佳。            |
| 21          | 2023-09-19      | JDK 21   | LTS      | LTS 版本；新增 subscription-based listener API、性能与稳定性改进。  |
| 24          | 2025-03-18      | JDK 24   | 非 LTS   | 引入 `--enable-native-access` 要求以访问本地渲染层；多显示器改进。   |
| 25          | 2025-09-16（计划）| JDK 25   | LTS（预期）| 下一个预期 LTS；继续完善 24 的特性，增强稳定性。                    |
| 26          | 2026-03（计划）  | JDK 26   | 非 LTS   | 短期支持版本，跟随最新 JDK 特性。                                   |

### 版本选择建议

- **生产环境 / 企业应用**：优先选择 **JavaFX 21 LTS** 或 **JavaFX 17 LTS**，稳定性与生态支持最佳。
- **新项目尝鲜**：可使用 **JavaFX 24+**，但需注意 `--enable-native-access` 标志要求。
...
| `--compress`        | 压缩级别（`zip-0` 到 `zip-9`，或 `zip-6`）。               |
